...
; TODO[
; Äîáàâèòü ïðîâåðêó òåãîâ ðàçìåòêè â áðèôèíãàõ.
; Óëó÷øèòü ïîääåðæêó ýëåâàòîðîâ.
; ...Óëó÷øèòü èíòåðôåéñ.
; ]TODO

; SDK Bugs[
; CopyRect - íå ðàáîòàåò.
...
#PElevator    ; Ìåíÿùèå âûñîòó áëîêè.
...
EndBlitz3d_() : End ; Òåïåðü âûõîäèì. Íó, â òåîðèè...
EndMacro
;}
;{ --Map management--
...
ColorText(TypeName)
If \Type = #BMeanie Or \Type = #BHunger Or \Type = #BFrostBite Or \Type = #BDesire ; Åñëè Ñóùíîñòü ìîæåò áûòü çàìîðîæåíà...
...
Text_(#TABleWidth / 2, #TABleHeight / 2, Text, 1, 1) ; Ñàìîå ãëàâíîå.
; ---Óãîëêè---
...
If HP > 1 ; Åñëè åñòü æèâûå èãðîêè.
For I = 1 To ToFix
...
\X = *Square\ArrayPos\X : \Y = *Square\ArrayPos\Y          ; Çàïîìèíàåì ïîçèöèþ.
If \Owner : System\Players(\Owner)\BeingsCount + 1 : EndIf ; Increse owner's beings count.
If IsReincarnated(Type) : \Reincarnated = #True            ; Óñòàíîâèòü ôëàã ðåèíêàðíàöèè.
ElseIf IsAscended(Type) : \Ascended = #True : \SpawningCost = #All : EndIf ; Óñòàíîâèòü ôëàã âîçíåñåíèÿ.
...
SetOn(\Entity, *Square\Entity) ; Ñòàâèì íà ïëàòôîðìó.
...
If ErrorText <> "" ; Åñëè ñîõðàíèòü íå óäàñòñÿ...
...
Procedure SaveMap(FName.S) ; Äîäåëàòü !
; Ïðîâåðêà âàëèäíîñòè êàðòû.
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
WriteLong(1, Len(System\LockCode))           ; Âïèñûâàåì äëèíó êîäà.
If System\LockCode : NewCode.s = InvalidatePass(System\LockCode)
WriteData(1, @NewCode, Len(NewCode))         ; Âïèñûâàåì êîä íà èçìåíåíèå.
EndIf : WriteLong(1, Len(System\PlayCode))   ; Âïèñûâàåì äëèíó êîäà.
If System\PlayCode : NewCode.s = InvalidatePass(System\PlayCode)
WriteData(1, @NewCode, Len(NewCode)) : EndIf ; Âïèñûâàåì êîä íà èãðó.
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
WriteData(1, @System\Level, SizeOf(LevelData))
WriteLong(1, Len(System\Story)) ; Çàïèñü áðèôèíãà êàðòû.
WriteData(1, @System\Story, Len(System\Story))
WriteLong(1, Len(System\AfterStory)) ; Çàïèñü ýïèëîãà êàðòû.
...
WriteLong(1, EntityPitch_(*Ptr)) ; Äðóãèå óãëû íå íóæíû.
Next
; Çàïèñàòü äàííûå êàíàëîâ (Dummy).
WriteLong(1, 0)
; Çàïèñàòü äàííûå ýôôåêòîâ ïåðåðîæäåíèÿ (Dummy).
WriteLong(1, 0)
; Çàïèñàòü äàííûå äîæäåâûõ èñêð (Dummy).
WriteLong(1, 0)
WriteLong(1, 0)
; Çàêðûòü ôàéë.
CloseFile(1)
...
Define FName.S
ShowPointer_()
If CheckSaving() = #False ; Åñëè âîçìîæíî ñîõðàíåíèå....
...
Case #PB_MessageRequester_Yes ; Ñîãëàñèå íà ñîõðàíåíèå.
If System\GUI\MapFName <> "" : SaveMap(System\GUI\MapFName)
Else : SaveMapRequester() : EndIf
Case #PB_MessageRequester_Cancel ; Îòìåíà äåéñòâèÿ.
ProcedureReturn #True
EndSelect
EndIf
EndProcedure
...
*Tube = CopyEntity_(System\MatrixTube)               ; Äåëàåì òðóáó.
AddElement(System\GUI\Matrix()) : System\GUI\Matrix() = *Tube ; Âíîñèì íà áàëàíñ.
SetOn(*Tube, Squares(X, Y)\TubesPiv)                          ; Ñòàâèì íà ìåñòî.
...
Reconnect(X, Y) ; Âîññîåäèíÿåì êëåòêè.
...
PositionEntity_(GetChild_(Elevator, 1), 0, (Scale - 1) * -#CellSize, 0, 0) ; Ñäâèã óçëîâ.
EndMacro
...
With *Cell ; Íà÷èíàåì îáðàáîòêó êëåòêè...
If SkipRemoval = #False ; Åñëè íóæíà ïåðåðàáîòêà.
Select \CellType ; Ñíà÷àëà óáèðàåì ñòàðîå.
Case #pVanilla : HideVisual(\Visuals(#AVanilla), *Cell\Gold) ; Ñêðûâàåì.
\CellType = #PB_Ignore : HandleConnections(XPos, YPos) ; Óáèâàåì òðóáû.
Case #pElevator  : HideEntity_(\Visuals(#AElevator))   ; Ñêðûòü ýëåâàòîð.
Case #pIgnorance : HideEntity_(\Visuals(#AIgnorance))  ; Ñêðûâàåì îáåëèñê.
EndSelect ; ...È ïîñëåäíåå (óáèâàåì ïîçîëîòó):
...
\CellType = NewType ; Ñòàâèì íîâûé òèï.
EndIf ; Ïðîäîëæàåì îáðàáîòêó:
Select \CellType ; Óñòàíàâëèâàåì ãðàôèêó ïî òèïó.
Case #pVanilla  : ShowVisual(\Visuals(#AVanilla), *Cell\Gold) ; Ñêðûâàåì.
HandleConnections(XPos, YPos)                         ; Ñòàâèì òðóáû íà ìåñòî.
Case #pElevator : ShowEntity_(\Visuals(#AElevator))   ; Ïîêàçàòü ýëåâàòîð.
Case #pIgnorance : ShowEntity_(\Visuals(#AIgnorance)) ; Ïîêàçàòü îáåëèñê.
If SkipRemoval = #False : HandleConnections(XPos, YPos) : EndIf
ChangePlatform(XPos, YPos, 0) ; Íà âñÿêèé ïîæàðíûé çàâîðà÷èâàåì â 0.
...
InitializeStructure(@System\GUI, GUIData) ; Äàáû àêòèâèðîâàòü ñïèñêè.
EndMacro
...
SetBuffer_(*Buffer) ; Óñòàíîâêà áóôôåðà.
...
For I = 1 To Params\PlayersCount ; Ïðîõîäèì ïî èãðîêàì...
...
For I = 1 To #MaxPlayers ; Ïðîõîäèì ïî èãðîêàì...
...
Procedure LoadMap(FName.S = #NewMap) ; Äîäåëàòü !
With System
...
If FName = #NewMap ; Îïðåäåëåíèå òèïà çàãðóçêè.
ShowPointer_() ; Âðåìåííî ïîêàçàòü êóðñîð ìûøè.
X = MGRequester(Params, \Options) ; Ïîëó÷åíèå äàííûõ îò ïîëüçîâàòåëÿ.
PerformOptions()
HidePointer_() ; Ñêðûòü êóðñîð ìûøè.
Select X
Case #True : Random = #True
Case #False : Quit() ; Ñðàçó âûõîäèì. Äà, òóò.
...
ProcedureReturn #False
EndIf
If ReadLong(0) = #False ; Íå ãðóçèì ñîõðàíåííûå èãðû.
LoadingErr("can't load game saves !")
CloseFile(0) : ProcedureReturn #False
EndIf ; Òåïåðü ïðîâåðÿåì çàùèòíûé êîä:
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
...
CloseFile(0) : ProcedureReturn #False
EndSelect : \LockCode = Code ; ÇÀïîìèíàåì êîä.
EndIf : I = ReadLong(0) : If I : Code = Space(I) : ReadData(0, @Code, I) ; ×èòàåì èãðîâîé êîä...
\PlayCode = InvalidatePass(Code) ; Èãðîâîé êîä.
EndIf ; Ïîêàçàòü ïðåäóïðåæäåíèå î íåñîõðàíåííûõ èçìåíåíèÿõ:
...
; Íà÷èíàåì ãåíåðàöèþ.
If Random ; Åñëè èäåò ñëå÷àéíûé ïîðÿäîê...
ConsumeParams() ; àíàëèçèðóåì äàííûå.
Else : ReadData(0, @\Level, SizeOf(LevelData))     ; Çàãðóçèòü ðàçìåð ïîëÿ èç ôàéëà.
I = ReadLong(0) : \Story = Space(I) : ReadData(0, @\Story, I)           ; Çàãðóçèòü áðèôèíã.
I = ReadLong(0) : \AfterStory = Space(I) : ReadData(0, @\AfterStory, I) ; Çàãðóçèòü ýïèëîã.
EndIf
...
*Platform = CopyEntity_(*Platform) ; Ñîçäàåì ïëàòôîðìó è îñíîâó íà áóäóùåå.
If Random : Scale = 1 ; Åñëè ñëó÷àéíàÿ ãåíåðàöèÿ.
...
*Cell\OwnData    = #Null
EndIf 
InitializeStructure(*Cell, Square) ; Ðå-èíèöèàëèçàöèÿ.
*Cell\Visuals(#AVanilla)     = *Platform ; Âïèñêà ïëàòôîðìû.
*Cell\Visuals(#AVanillaPure) = *Platform ; Âïèñêà ïëàòôîðìû.
; Íàñòðîéêà àëüòåðíàòèâ.
...
TranslateEntity_(*Elevator, 0, -#HalfCell - #SquareLuft, 0) ; Íåáîëüøîå ñìåùåíèå.
CopyEntity_(\NodeCubes, *Elevator) : *Cell\Visuals(#AIgnorance)    = *Ignorance ; Îáåëèñê.
*Cell\Visuals(#AElevator) = *Elevator    : *Cell\Visuals(#AElevatorPure) = *Elevator  ; Ýëåâàòîð.
Define *Gold = CopyEntity_(\PlatformGold, *Platform) ; Ñîçäàíèå çîëîòîé ïëàòôîðìû.
*Cell\Visuals(#AVanillaGold) = *Gold : HideEntity_(*Gold)  ; Âïèñêà çîëîòîé ïëàòôîðìû.
*Gold = CopyEntity_(\ElevatorGold, *Square) : CopyEntity_(\NodeCubesGold, *Gold) 
SetOn(*Gold, *Elevator) : *Cell\Visuals(#AElevatorGold) = *Gold : HideEntity_(*Gold) ; Âïèñêà çîëîòà.
; Óñòàíîâêà ðàçìåðà ïëàòôîðìû:
*Cell\TubesPiv = CreatePivot_(*Platform)
Define Gold = *Cell\Gold : *Cell\Gold = #False ; Ñîõðàíÿåì íà áóäóùåå.
If *Cell\CellType : EntityAlpha_(*Platform, 0) ; Ñðàçó ñêðûâàåì.
RestructureCell(X, Y, *Cell\CellType, #True)   ; Ðåñòðóêòóðèçóåì.
EndIf : ChangePlatform(X, Y, Gold, Scale) ; Ðåìàñøòàáèðóåì.
; Ìàòðèöà.
If Y = 0 : MatrixTubes(X, Y, -90) : EndIf
If X = 0 : MatrixTubes(X, Y, 0)   : EndIf
Type = ~Type ; Êðàñíî-çåëåííîå.
Next X
If \Level\Width % 2 = 0 : Type = ~Type : EndIf
Next Y
; --Players setup--
If Random ; Åñëè ñîçäàåì íîâóþ êàðòó...
; Ñîçäàòü çàäàííûõ ïîëüçîâàòåëåì èãðîêîâ.
ReDim \Players.Player(Params\PlayersCount)
\Level\PlayersCount = Params\PlayersCount
UpdatePlayers() ; Ïðàâèì äàííûå èãðîêîâ.
Else ; Çàãðóçèòü äàííûå î èãðîêàõ èç ôàéëà.
ToFix = ReadLong(0) ; Êîë-âî èãðîêîâ.
ReDim \Players.Player(ToFix)
For I = 1 To ToFix ; Îáðàáàòûâàåì êàæäîãî.
ReadData(0, \Players(I), SizeOf(Player))
\Players(I)\BeingsCount = 0
\Players(I)\TotalEnergy = 0
Next I ; Çàãðóçèòü äàííûå î êîììàíäàõ...
...
; -Beings-
If Random = #False ; Çàãðóçêà äàííûõ î Ñóùíîñòÿõ èç ôàéëà
...
Next I
; Çàãðóçêà äàííûõ î êâàíòàõ èç ôàéëà (Dummy).
ReadLong(0)
EndIf
If Random ; Íàñòðàèâàåì êàìåðó.
PositionEntity_(\CamPiv, #MapBoundsGap, 0, -#MapBoundsGap)
RotateEntity_(\CamPiv, 0, 45, 0)  ; Iso-view.
SetOn(\Camera, \CamPiv)     ; Ñòàâèì â íà÷àëî.
...
Next
; Çàãðóçêà äàííûõ î êàíàëàõ èç ôàéëà (Dummy).
ReadLong(0)
; Çàãðóçêà äàííûõ î ýôôåêòàõ ïåðåðîæäåíèÿ èç ôàéëà (Dummy).
ReadLong(0)
; Çàãðóçêà äàííûõ î èñêðàõ èç ôàéëà (Dummy).
ReadLong(0)
ReadLong(0)
CloseFile(0)
EndIf
...
EndIf
If NewType <> *Being\Type ; Åñëè Ñóùíîñòü ìîæíî ðåèíêàðíèðîâàòü...
ExcludeBeing(*Being)
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
If System\GUI\PickedSquare : *Being = System\GUI\PickedSquare\Being ; Åñëè âûäåëåíà ïëàòôîðìà...
If CheckBeing(*Being) ; Åñëè íà íåé åñòü Ñóùíîñòü...
; Îòîáðàæåíèå òàáëèöû.
...
Define I, *Texture
; -System init-
While CreateMutex_(0, 0, "=[Flow.Editor]=") = #Null : Delay(100) : Wend ; Èäåíòèôèêàòîð.
...
; --Options--
If OpenOptionsFile() = #False ; Åñëè íå óäàëîñü çàãðóçèòü íàñòðîåê...
...
Select System\GUI\Input
Case #PB_MB_Left ; Èçìåíèòü ñóùíîñòü íà ïëàòôîðìå.
If *PSquare <> #Null
...
EndIf
Case #PB_MB_Right ; Óäàëèòü Ñóùíîñòü íà ïëàòôîðìå.
*Being = SelectedBeing() : If *Being : RemoveBeing(*Being) : VizualizeChanges() : EndIf
Case #PB_Key_Space ; Èçìåíèòü âûñîòó ïëàòôîðìû.
If *PSquare <> #Null
...
EndIf
Case #PB_Key_Add ; Óâåëè÷èòü ýíåðãåòè÷åñêèé ïîòåíöèàë Ñóùíîñòè.
*Being = SelectedBeing()
...
EndIf
EndIf
Case #PB_Key_Subtract ; Óìåíüøèòü ýíåðãåòè÷åñêèé ïîòåíöèàë Ñóùíîñòè.
*Being = SelectedBeing()
If *Being
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
Case #PB_Key_G
If *PSquare ; Åñëè ÷òî-òî ïîäñâå÷åíî...
If *PSquare\CellType = #pIgnorance
Define NewGold = (*PSquare\Gold + 1) % 4 ; Ïîâîðîò íà ìåñòå.
...
If *PSquare\Being : ScriptDataBoard(*PSquare\Being) : EndIf ; Îòðàæàåì.
VizualizeChanges()
EndIf
Case #PB_Key_Home
*Being = SelectedBeing()
...
Else : *Being = System\GUI\SelectedPlayer : EndIf ; Âûñòàâëÿåì áóäóùåãî õîçÿèíà.
...
Select System\GUI\Input
Case #PB_Key_Escape : GotoMainPanel() ; Âîçâðàò íà ïàíåëü ñîçäàíèé êàðòû.
...
; ^Çàäåðæêà, åñëè ñêîðîñòü ïðîãðàììû ñëèøêîì áîëüøàÿ.^
ForEver
